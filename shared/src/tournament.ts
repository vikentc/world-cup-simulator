...
export function initializeTournament(
  ...
  const clonedTeams = JSON.parse(JSON.stringify(teams)) as Record<string, Team>;
  ...
  Object.entries(GROUPSTAGE_GROUPS).forEach(([groupId, teamIds]) => {
    const table: GroupTableEntry[] = teamIds.map((teamId) => ({
      teamId,
 ...
 */
export function recalculateGroupTable(
  group: Group,
  completedMatches: MatchSimulationState[]
): void {
  // Reset table entries
 ...
 */
export function getBestThirdPlaceTeams(groups: Group[], teams: Record<string, Team>): string[] {
  const thirdPlaceEntries: { entry: GroupTableEntry; groupId: string }[] = [];
  ...
  thirdPlaceEntries.sort((a, b) => {
    ...
    if (b.entry.goalDifference !== a.entry.goalDifference) return b.entry.goalDifference - a.entry.goalDifference;
    if (b.entry.goalsFor !== a.entry.goalsFor) return b.entry.goalsFor - a.entry.goalsFor;
...
interface ThirdPlaceAssignment {
  ...
  allowedThirdPlaceGroups: string[];
  nodeId: number;
}

const assignments: ThirdPlaceAssignment[] = [
...
function matchThirdPlaceTeams(
  qualifiedThirds: { groupId: string; teamId: string }[],
  ...
  function backtrack(index: number): boolean {
    ...
    for (const third of qualifiedThirds) {
      if (used.has(third.teamId)) continue;
  ...
  for (const slot of assignments) {
    const matched = qualifiedThirds.find(
      (third) => !used.has(third.teamId) && third.groupId !== slot.winnerGroupId
    );
    if (matched) {
...
export function generateRoundOf32(
...
): Record<number, KnockoutMatchNode> {
  ...
  const nodes: Record<number, KnockoutMatchNode> = {};
 ...
 */
export function propagateKnockoutWinners(tournament: TournamentState): void {
 ...
 */
export function updateTournamentPlayerStats(
  tournament: TournamentState,
  matches: MatchSimulationState[]
): void {
 ...
 */
export function applyPostMatchRosterUpdates(
  tournament: TournamentState,
  matches: MatchSimulationState[]
): void {
  matches.forEach((match) => {
    ...
    const homePlayedIds = new Set<number>();
    const awayPlayedIds = new Set<number>();

    // Final players on the pitch
    match.players.forEach((p) => {
      if (p.teamId === homeTeam.id) homePlayedIds.add(p.playerId);
      if (p.teamId === awayTeam.id) awayPlayedIds.add(p.playerId);
    });

    // Plus players who were substituted out
    match.events.forEach((ev) => {
      if (ev.type === 'SUBSTITUTION' && ev.playerId) {
        if (ev.teamId === homeTeam.id) homePlayedIds.add(ev.playerId);
        if (ev.teamId === awayTeam.id) awayPlayedIds.add(ev.playerId);
      }
    });

    // Keep track of cards and injuries in this match to apply suspensions/injuries
    const matchCards: Record<number, { yellow: number; red: boolean }> = {};
    const matchInjuries = new Set<number>();

    match.events.forEach((ev) => {
      if (!ev.playerId) return;
      if (ev.type === 'YELLOW_CARD') {
        if (!matchCards[ev.playerId]) matchCards[ev.playerId] = { yellow: 0, red: false };
    ...
    const updateTeamRoster = (
      ...
      team.players.forEach((player) => {
        ...
        if (won) {
          ...
          if (eloDiff > 100) morale += 3;
          else if (eloDiff < -100) morale -= 2;
          else morale += 1;
        }
        ...
        if (played) {
          ...
          if (cards) {
            if (cards.red) morale -= 8;
            if (cards.yellow > 0) morale -= cards.yellow * 2;
          }
        }
