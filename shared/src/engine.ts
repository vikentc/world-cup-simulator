...
const GOAL_CENTER_LEFT: Vector2D = { x: 0, y: 34 };
const GOAL_CENTER_RIGHT: Vector2D = { x: 105, y: 34 };

/**
 * Selects the starting 11 players based on team formation and player positions,
 * and assigns them roles on the pitch.
 */
export function selectStartingLineup(team: Team): { player: Player; role: string; position: PlayerPosition }[] {
  const formation = team.tactics.formation;
  const roles = FORMATION_ROLES[formation];

  const squad = [...team.players];
  const selected: { player: Player; role: string; position: PlayerPosition }[] = [];
...
export function initializeMatchState(
  ...
  homeTeam: Team,
  awayTeam: Team,
  round?: 'GROUP_STAGE' | 'R32' | 'R16' | 'QF' | 'SF' | 'FINAL'
): MatchSimulationState {
  // Deep clone to avoid mutating parent tournament references directly during simulation
  const clonedHome = JSON.parse(JSON.stringify(homeTeam)) as Team;
  const clonedAway = JSON.parse(JSON.stringify(awayTeam)) as Team;
  ...
  if (round === 'R32') pressureMult = 1.1;
  else if (round === 'R16') pressureMult = 1.2;
  else if (round === 'QF') pressureMult = 1.3;
  else if (round === 'SF') pressureMult = 1.4;
  else if (round === 'FINAL') pressureMult = 1.5;
  ...
  const players: PlayerOnPitchState[] = [];

  // Map home team to pitch
  homeStarting.forEach(({ player, role, position }) => {
    const defaultCoords = getTacticalTargetPosition(
      role,
      clonedHome.tactics.formation,
      true,
      { x: 52.5, y: 34 },
      clonedHome.tactics,
      null,
...
function triggerSetPiece(state: MatchSimulationState, type: MatchEventType, teamId: string) {
...
function addCommentary(state: MatchSimulationState, type: MatchEventType, text: string) {
  const minute = Math.floor(state.elapsedSeconds / 60);
  const second = Math.floor(state.elapsedSeconds % 60);
...
function getCoachName(teamId: string, teamName: string): string {
...
function addCoachShout(
  state: MatchSimulationState,
  teamId: string,
  reactionType: 'GOAL_SCORED' | 'GOAL_CONCEDED' | 'SHOT_MISSED' | 'SHOT_CONCEDED' | 'MISTAKE_SELF' | 'MISTAKE_OPPONENT' | 'TACTICAL' | 'HAPPY' | 'CURSE'
) {
...
function getPossessionTeamId(state: MatchSimulationState): string | null {
 ...
 */
export function simulateTick(state: MatchSimulationState): MatchSimulationState {
    ...
    if (state.activeShot.ticksToTarget <= 0) {
      ...
      if (activeShot.isSaved && activeShot.gkId && state.ball.ownerId === null) {
        ...
        if (gk) {
          ...
          if (isHome) state.stats.savesHome++;
          else state.stats.savesAway++;
  ...
  if (physicsResult.outOfBounds) {
    if (physicsResult.isGoal && physicsResult.scoringTeamId) {
      ...
      addCommentary(
        state,
        'GOAL',
        `GOAL!!! Beautiful finish by ${goalScorer}! The stadium erupts! ${state.homeTeam.name} ${state.homeScore} - ${state.awayScore} ${state.awayTeam.name}.`
      );
      ...
      if (physicsResult.reason === 'CORNER') {
        ...
        if (takingTeam === state.homeTeam.id) state.stats.cornersHome++;
        else state.stats.cornersAway++;
  ...
  if (state.ball.ownerId === null && state.ball.height < 1.5 && (!state.ball.noControlTicks || state.ball.noControlTicks <= 0)) {
    ...
    let interceptingPlayer: PlayerOnPitchState | null = null;
    ...
    if (interceptingPlayer) {
      ...
      const player = interceptingPlayer as PlayerOnPitchState;
      ...
      if (isInterception) {
        ...
        if (isHome) state.stats.interceptionsHome++;
        else state.stats.interceptionsAway++;
...
function executeSetPieceKick(state: MatchSimulationState) {
...
function executeGKClearance(state: MatchSimulationState, owner: PlayerOnPitchState, attackingRight: boolean, isHome: boolean) {
  const ball = state.ball;
  ball.ownerId = null;
  const targetDir = attackingRight ? 1 : -1;
 ...
 */
function handlePlayerDecisions(state: MatchSimulationState) {
  const ball = state.ball;
  if (ball.ownerId === null) return; // Loose ball, no decisions
  ...
  if (owner.position === 'GK') {
    ...
    const passingOptions: { player: PlayerOnPitchState; score: number }[] = [];
  ...
  if (pressers.length > 0) {
    ...
    if (tackleScore > dribbleScore) {
      ...
      if (isInterception) {
        ...
        if (isHome) state.stats.interceptionsAway++;
        else state.stats.interceptionsHome++;
  ...
  if (inRange && distToGoal < maxShotDist) {
    ...
    if (scoreDiff >= 3) {
      if (scoreDiff === 3) shotChance *= 0.60;      // 40% reduction
      else if (scoreDiff === 4) shotChance *= 0.30; // 70% reduction
      else shotChance *= 0.08;                      // 92% reduction (very patient, almost stops shooting unless golden chance)
    }
  ...
  teammates.forEach((tm) => {
    ...
    if (d > 5 && d < 35) {
      ...
      const formsTriangle = teammates.some((other) => {
        if (other.playerId === tm.playerId) return false;
        if (other.position === 'GK') return false; // GK doesn't count for passing triangles
...
function checkOffside(state: MatchSimulationState, passer: PlayerOnPitchState, receiver: PlayerOnPitchState): boolean {
  const isHome = passer.teamId === state.homeTeam.id;
  const oppTeamId = isHome ? state.awayTeam.id : state.homeTeam.id;

  // 1. Offside is only possible in the opponent's half
  if (isHome) {
    if (receiver.pos.x <= 52.5) return false;
  } else {
    if (receiver.pos.x >= 52.5) return false;
  }
  ...
  if (isHome) {
    if (receiver.pos.x <= state.ball.pos.x) return false;
  } else {
    if (receiver.pos.x >= state.ball.pos.x) return false;
  }
...
function sendPlayerOff(state: MatchSimulationState, player: PlayerOnPitchState, reason: 'direct' | 'double_yellow') {
...
function showYellowCard(state: MatchSimulationState, player: PlayerOnPitchState) {
  const team = player.teamId === state.homeTeam.id ? state.homeTeam : state.awayTeam;
  const teamPlayer = team.players.find(p => p.id === player.playerId);
...
function executeFoul(state: MatchSimulationState, defender: PlayerOnPitchState, victim: PlayerOnPitchState) {
...
function triggerInjury(state: MatchSimulationState, player: PlayerOnPitchState, type: 'fatigue' | 'tackle' | 'foul') {
  const team = player.teamId === state.homeTeam.id ? state.homeTeam : state.awayTeam;
  const teamPlayer = team.players.find(p => p.id === player.playerId);
  if (teamPlayer) {
...
function performSubstitutionForPlayer(state: MatchSimulationState, oldPlayer: PlayerOnPitchState, isInjury: boolean) {
  ...
  let chosenSub: Player | undefined;

  if (isInjury) {
    const samePos = bench.filter(p => p.position === oldPlayer.position);
    if (samePos.length > 0) {
      samePos.sort((a, b) => {
        const rating = (p: Player) => p.attributes.passing + p.attributes.composure + p.attributes.decisions;
        return rating(b) - rating(a);
      });
      chosenSub = samePos[0];
    } else {
      bench.sort((a, b) => {
        const rating = (p: Player) => p.attributes.passing + p.attributes.composure;
        return rating(b) - rating(a);
      });
      chosenSub = bench[0];
    }
...
function checkSubstitutions(state: MatchSimulationState) {
  const minute = Math.floor(state.elapsedSeconds / 60);
  if (minute < 55) return;
...
function checkTeamSubs(state: MatchSimulationState, teamId: string) {
  const team = teamId === state.homeTeam.id ? state.homeTeam : state.awayTeam;
  const subsMade = state.events.filter(e => e.type === 'SUBSTITUTION' && e.teamId === team.id).length;
  if (subsMade >= 5) return;
 ...
 */
function executePass(state: MatchSimulationState, passer: PlayerOnPitchState, receiver: PlayerOnPitchState) {
  ...
  if (passer.teamId === state.homeTeam.id) {
    state.stats.passesHome++;
    if (Vector.dist(receiver.pos, targetCoords) < 3.0) state.stats.passesCompletedHome++;
  } else {
    state.stats.passesAway++;
    if (Vector.dist(receiver.pos, targetCoords) < 3.0) state.stats.passesCompletedAway++;
  }
  ...
  const formsTriangle = teammates.some((other) => {
    if (other.playerId === receiver.playerId) return false;
    if (other.position === 'GK') return false;
 ...
 */
function executeShot(state: MatchSimulationState, shooter: PlayerOnPitchState, goalCenter: Vector2D, dist: number, isPenalty = false) {
  const ball = state.ball;
  ball.ownerId = null;

  const isHome = shooter.teamId === state.homeTeam.id;
  if (isHome) state.stats.shotsHome++;
  else state.stats.shotsAway++;
  ...
  if (isHome) state.stats.xgHome += xG;
  else state.stats.xgAway += xG;
  ...
  if (isOffTarget) {
    ...
    if (isHome) state.stats.shotsOnTargetHome++;
    else state.stats.shotsOnTargetAway++;
    detailsText = 'Shot is on target!';
  }

  // 3. Set ball velocities
  const targetPos: Vector2D = { x: goalCenter.x, y: finalTargetY };
  ...
  let isBlocked = false;
  let blockingDefender: PlayerOnPitchState | null = null;
 ...
 */
export function getSetPieceTargetPosition(
  state: MatchSimulationState,
  p: PlayerOnPitchState,
  ...
  return getTacticalTargetPosition(
    p.role,
    p.teamId === state.homeTeam.id ? state.homeTeam.tactics.formation : state.awayTeam.tactics.formation,
    p.teamId === state.homeTeam.id,
    ball.pos,
    p.teamId === state.homeTeam.id ? state.homeTeam.tactics : state.awayTeam.tactics,
    null,
    p.teamId
  );
}
 ...
 */
function updatePlayerMovement(state: MatchSimulationState, forceDefensiveReset: boolean) {
  const ball = state.ball;
  ...
  state.players.forEach((p) => {
    ...
    if (isOwner) {
      ...
      if (activeSetPiece) {
        ...
        target = getTacticalTargetPosition(
          p.role,
          p.teamId === state.homeTeam.id ? state.homeTeam.tactics.formation : state.awayTeam.tactics.formation,
          p.teamId === state.homeTeam.id,
          { x: 52.5, y: 34 },
...
function getDistanceToSegment(p: Vector2D, a: Vector2D, b: Vector2D): number {
  const ab = Vector.sub(b, a);
  const ap = Vector.sub(p, a);
  const ab2 = Vector.dot(ab, ab);
  if (ab2 === 0) return Vector.dist(p, a);
 ...
 */
export function simulateMatchInstant(state: MatchSimulationState): MatchSimulationState {
  ...
  if (homeStyle === 'Gegenpress') homeXG *= 1.1;
  else if (homeStyle === 'Possession') homeXG *= 0.95;
  else if (homeStyle === 'Low Block') homeXG *= 0.85;
  else if (homeStyle === 'Direct Play') homeXG *= 1.05;

  if (awayStyle === 'Gegenpress') awayXG *= 1.1;
  else if (awayStyle === 'Possession') awayXG *= 0.95;
  else if (awayStyle === 'Low Block') awayXG *= 0.85;
  else if (awayStyle === 'Direct Play') awayXG *= 1.05;
  ...
  if (homeStyle === 'Possession' && awayStyle !== 'Possession') possessionHome += 4;
  if (awayStyle === 'Possession' && homeStyle !== 'Possession') possessionHome -= 4;
  if (homeStyle === 'Low Block') possessionHome -= 3;
  if (awayStyle === 'Low Block') possessionHome += 3;
  ...
  if (homeStyle === 'Gegenpress') homeShotsBase *= 1.15;
  if (homeStyle === 'Possession') homeShotsBase *= 0.85;
  if (awayStyle === 'Gegenpress') awayShotsBase *= 1.15;
  if (awayStyle === 'Possession') awayShotsBase *= 0.85;
  ...
  if (homeStyle === 'Possession') homeAccuracy += 0.05;
  if (awayStyle === 'Possession') awayAccuracy += 0.05;
  ...
  function pickScorer(teamPlayers: any[]): any | undefined {
    if (!teamPlayers || teamPlayers.length === 0) return undefined;
    const outfield = teamPlayers.filter(p => p.position !== 'GK' && !p.redCarded);
    if (outfield.length === 0) return teamPlayers[0];
    const weights = outfield.map(p => {
      if (p.position === 'FW') return 10;
      if (p.position === 'MF') return 5;
      return 1;
    });
  ...
  function pickAssister(teamPlayers: any[], scorerId: number): any | undefined {
    if (!teamPlayers || teamPlayers.length === 0) return undefined;
    const outfield = teamPlayers.filter(p => p.position !== 'GK' && p.id !== scorerId && !p.redCarded);
    if (outfield.length === 0) return undefined;
    const weights = outfield.map(p => {
      if (p.position === 'MF') return 10;
      if (p.position === 'FW') return 6;
      return 2;
    });
    const totalWeight = weights.reduce((a, b) => a + b, 0);
    let r = Math.random() * totalWeight;
  ...
  function pickCardedPlayer(teamPlayers: any[]): any | undefined {
    if (!teamPlayers || teamPlayers.length === 0) return undefined;
    const active = teamPlayers.filter(p => !p.redCarded);
    if (active.length === 0) return undefined;
    const weights = active.map(p => {
      if (p.position === 'DF') return 8;
      if (p.position === 'MF') return 6;
      if (p.position === 'FW') return 4;
      return 1;
    });
    const totalWeight = weights.reduce((a, b) => a + b, 0);
    let r = Math.random() * totalWeight;
  ...
  const completedState: MatchSimulationState = {
    ...state,
    status: 'COMPLETED',
    homeScore,
    awayScore,
