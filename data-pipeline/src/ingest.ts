...
async function runETL() {
  ...
  const scrapedElos: Record<string, number> = {};
  ...
  try {
    ...
    $('tr').each((_: any, elem: any) => {
      ...
      if (countryName && !isNaN(rating)) {
        ...
        const match = WC_NATIONS.find((n: any) => n.name.toLowerCase() === countryName.toLowerCase());
        if (match) {
    ...
    try {
      ...
      for (const nation of WC_NATIONS) {
        ...
        if (res.rows.length > 0) {
          ...
          await client.query(
            'UPDATE db_teams SET data = $1 WHERE id = $2',
            [JSON.stringify(team), nation.id]
          );
