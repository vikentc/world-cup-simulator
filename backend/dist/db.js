    ...
    try {
        ...
        if (count === 0) {
            ...
            for (const team of Object.values(generatedTeams)) {
                await client.query('INSERT INTO db_teams (id, name, data) VALUES ($1, $2, $3)', [team.id, team.name, JSON.stringify(team)]);
            }
    ...
    if (isPostgresReady && pool) {
        try {
            if (match.matchId === 0 || !match.matchId) {
                // Insert new match
                const res = await pool.query(`INSERT INTO db_matches (home_team_id, away_team_id, status, home_score, away_score, elapsed_seconds, data)
           VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING id`, [
                ...
                await pool.query(`UPDATE db_matches SET status = $1, home_score = $2, away_score = $3, elapsed_seconds = $4, data = $5
           WHERE id = $6`, [
                    match.status,
                    match.homeScore,
                    match.awayScore,
    ...
    if (isPostgresReady && pool) {
        try {
            if (tournament.id === 0 || !tournament.id) {
                const res = await pool.query(`INSERT INTO db_tournaments (name, status, current_round, data)
           VALUES ($1, $2, $3, $4) RETURNING id`, [
                ...
                await pool.query(`UPDATE db_tournaments SET status = $1, current_round = $2, data = $3
           WHERE id = $4`, [
                    tournament.status,
                    tournament.currentRound,
                    JSON.stringify(tournament),
