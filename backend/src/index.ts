...
const activeSimulations = new Map<number, {
  ...
  speed: number; // 0 = paused, 1 = 1x, 5 = 5x, 10 = 10x
...
app.post('/api/tournaments', async (req, res) => {
  ...
  try {
    ...
    const matchesToSave: Promise<any>[] = [];
...
app.post('/api/tournaments/:id/simulate-round', async (req, res) => {
  ...
  try {
    ...
    if (tournament.currentRound === 'GROUP_STAGE') {
      ...
      const matchesByRound: Record<number, { matchId: number; homeTeamId: string; awayTeamId: string }[]> = {
        1: [],
        2: [],
        3: [],
      };
      ...
      for (const group of tournament.groups) {
        for (const mId of group.matches) {
          ...
          if (match) {
            ...
            const sched = GROUPSTAGE_SCHEDULE.find(
              (s) =>
                s.group === group.id &&
      ...
      for (const group of tournament.groups) {
        const groupMatches: MatchSimulationState[] = [];
        for (const mId of group.matches) {
          const m = allSimulatedMatches.find((x) => x.matchId === mId);
          if (m) groupMatches.push(m);
        }
      ...
      const simulatedKnockoutMatches: MatchSimulationState[] = [];
      ...
      for (const node of currentRoundNodes) {
        if (node.winnerId) {
          ...
          if (node.matchId) {
            ...
            if (match) simulatedKnockoutMatches.push(match);
          }
          continue;
        }
      ...
      if (tournament.currentRound === 'FINAL') {
        ...
        const nextRoundMap: Record<string, 'R16' | 'QF' | 'SF' | 'FINAL'> = {
          R32: 'R16',
          R16: 'QF',
          QF: 'SF',
          SF: 'FINAL',
...
function startMatchSimulationLoop(matchId: number) {
